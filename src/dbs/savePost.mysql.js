...
class SaveListQuery extends QueryBase {
	...
	async getSaveList(nameList, userId) {
		const query =
			'SELECT * FROM SAVELIST WHERE nameSaveList = ? AND userId = ?';
		const existingSaveList = await this.dbInstance.hitQuery(query, [
			nameList,
			userId,
		]);
	...
	async getSaveListById(saveListId, userId) {
		const query =
			'SELECT * FROM SAVELIST WHERE saveListId = ? AND userId = ?';
		const existingSaveList = await this.dbInstance.hitQuery(query, [
			saveListId,
			userId,
		]);
...
class SavePostQuery extends QueryBase {
	...
	async getPostFromSaveList(savelistId, postId) {
		const query =
			'SELECT * FROM SAVELIST_POST WHERE saveListId = ? AND postId = ?';
		const existingSavePost = await this.dbInstance.hitQuery(query, [
			savelistId,
			postId,
		]);
	...
	async getPostsBySavelistId(savelistId) {
		const queryGetNumsPost = `SELECT COUNT(*) as total_record
                                  FROM SAVELIST_POST
                                  WHERE saveListId = ?`;
		const coutingResult = await this.dbInstance.hitQuery(queryGetNumsPost, [
			savelistId,
		]);
	...
	async saveNewPost(userId, nameList, postId) {
		...
		const existingSaveList = await this.saveListQuery.getSaveList(
			nameList,
			userId
		);
		let saveListId = null;
		if (!existingSaveList) {
			saveListId = uuidv4();
			console.log(
				`DEBUG: create new SaveList ${nameList} with id ${saveListId}`
			);
			await this.saveListQuery.createNewSaveList(
				saveListId,
				nameList,
				userId
			);
			...
			console.log(
				`DEBUG: SaveList ${nameList} existed with id ${saveListId}`
			);
		}
		...
		if (!(await this.getPostFromSaveList(saveListId, postId))) {
			...
			console.warn(
				`The post ${postId} was saved in ${saveListId} already`
			);
			return await this.saveListQuery.getSaveListById(saveListId, userId);
		}
	}
	...
	async unsavePost(userId, savelistId, postId) {
		// check if the nameList is existing or not, if not create new
		const existingSaveList = await this.saveListQuery.getSaveListById(
			savelistId,
			userId
		);
		if (!existingSaveList) {
	...
	async getSavedPost(saveListId, userId) {
		...
		savePosts = await this.getPostsBySavelistId(
			existingSaveList.saveListId
		);
		return { savePosts };
	}
	...
	async deleteSavePostById(saveListId, userId) {
		...
		if (!existingSaveList) {
			throw new BadRequestError(
				`The savelist with id ${saveListId} does not exist`
			);
		}
		const query =
			'DELETE FROM SAVELIST WHERE saveListId = ? AND userId = ?';
		await this.dbInstance.hitQuery(query, [saveListId, userId]);
	}
}
