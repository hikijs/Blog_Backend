...
class ImageData extends QueryBase {
	...
	async getImageFromUrl(imageUrl) {
		try {
			...
			const imageId = await this.dbInstance.hitQuery(getImageInserted, [
				imageUrl,
			]);
			return imageId;
		} catch (error) {
	...
	async upSertImage(imageUrl, topic, userId, postId) {
		try {
			if (topic == 'avatar') {
				const queryAvatar =
					'SELECT * FROM IMAGE WHERE userId = ? AND topic = ?';
				const avatarExisting = await this.dbInstance.hitQuery(
					queryAvatar,
					[userId, topic]
				);
				if (avatarExisting.length == 1) {
					...
					const query =
						'UPDATE IMAGE SET imageUrl = ? WHERE userId = ? and topic = ?';
					...
					console.log(
						'UPDATE: the existing image in db was updated successfully'
					);
					return;
				}
			} else if (topic == 'thumnail') {
				const queryThumbnail =
					'SELECT * FROM IMAGE WHERE postId = ? AND topic = ?';
				const thumbnailExisting = await this.dbInstance.hitQuery(
					queryThumbnail,
					[postId, topic]
				);
				if (thumbnailExisting.length == 1) {
					// FIXME should delete image on disk also
					const query =
						'UPDATE IMAGE SET imageUrl = ? WHERE postId = ? and topic = ?';
					const result = await this.dbInstance.hitQuery(query, [
						imageUrl,
						postId,
						topic,
			...
			// inserted new
			const query =
				'INSERT INTO IMAGE (imageId, imageUrl, topic, postId, userId)\
                        VALUES (UUID(), ?, ?, ?, ?)';
			const result = await this.dbInstance.hitQuery(query, [
				imageUrl,
				topic,
