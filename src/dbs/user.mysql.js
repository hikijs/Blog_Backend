...
class UserQuery extends QueryBase {
	...
	async getNonOauthUserByMail(email) {
		try {
			const query =
				'SELECT * FROM USER WHERE email = ? AND oauth_provider = FALSE';
	...
	async getOauthUserByEmail(email, oauthProvider) {
		try {
			...
			if (results.length == 0) {
				...
				throw new Error(
					`More than one user of oauth provider ${oauthProvider} use ${email}`
				);
			}
		} catch (error) {
			console.log(error);
	...
	async updateUserProfile(queriesData, userId) {
		const { query, queryParams, emailChange } =
			SqlBuilder.dynamicSqlForUpdateUserByUserId(queriesData, userId);
	...
	async updateVerifiedStatus(status, userId) {
		...
		await this.dbInstance.hitQuery(query, [status, userId]);
	}
}
