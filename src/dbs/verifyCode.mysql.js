...
class VerifyCodeQuery extends QueryBase {
	...
	async checkCodeExistOrNot(code, userId, typeCode) {
		try {
			const query =
				'SELECT *  FROM VERIFYCODE WHERE code = ? AND userId = ? AND typeCode = ?';
		...
		try {
			...
			try {
				if (existingCode) {
					...
					query =
						'UPDATE VERIFYCODE SET code = ? , expireTime = ?, typeCode = ?  \
                          WHERE codeId = ?';
					...
					query =
						'INSERT INTO VERIFYCODE (codeId, code, expireTime, typeCode, userId) \
                  VALUES (UUID(), ?, ?, ?, ?)';
					await this.dbInstance.hitQuery(query, [
						code,
						expireDate,
				...
				const verifyCodeSql =
					'SELECT * FROM VERIFYCODE WHERE userId = ?';
				const verifyCode = await this.dbInstance.hitQuery(
					verifyCodeSql,
					[userId]
				);
