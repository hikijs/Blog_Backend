...
class NotifyAbstract {
	...
	async updateDeliveryData() {
		...
		const receiverData = await UserQuery.getBasicUserDataById(
			this.receiverId
		);
	...
	async publishNotification() {
		const notifyMq = await RabbitMq.getInstance(
			NOTIFICATION_CONFIG?.EXCHANGES?.notify
		);
		const notifyJson = await this.toJsonObject();
		console.log(notifyJson);
		notifyMq.publishObject(
			notifyJson,
			NOTIFICATION_CONFIG?.NOTIFY_QUEUES?.notify
		);
	}
}
...
class NotifyManager {
	static triggerNotify(type, senderId, receiverId) {
		...
		const notificer = notifyFactory.createNotify(
			type,
			senderId,
			receiverId
		);
